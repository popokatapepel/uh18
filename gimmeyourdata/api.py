...
class ChefkochApi:
    ...
    def get_user(self, user_id):
        ...
        if len(title) >= 1:
            if "Keine oder ungueltige User-ID" in title[0].text.strip():
        ...
        for section in profile_sections:
            ...
            if section.text.strip().find('Ueber mich') >= 0:
            ...
            if section.text.strip().find('Schritt-fuer-Schritt-Anleitungen') >= 0:
                user['Anzahl-Schritt-fuer-Schritt-Anleitungen'] = re.findall(r'\d+', section.text.strip())[0]
                user['Schritt-fuer-Schritt-Anleitungen'] = self.get_step_by_step_guides(user_id)
